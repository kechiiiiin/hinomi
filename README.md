...
ログイン時に自動起動させたいときは、メニューバーの炎アイコン → **「ログイン時に起動」** をオンにしてください（`SMAppService` でシステム設定のログイン項目に登録します）。`~/Applications/hinomi.app` として起動している必要があります（`make install` 済みなら満たしています）。
...
| `soundsEnabled` | `true` | 効果音を鳴らすか。`false` で音名の設定を保ったまま黙らせる（メニューの「効果音」と同じ） |
